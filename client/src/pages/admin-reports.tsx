...
import { format, parse, startOfWeek, getDay, startOfWeek as startOfWeekDate, addDays } from "date-fns";
...
export default function AdminReports() {
  ...
  const sessions = useMemo(() => {
    let filtered = selectedClass
      ? allSessions.filter((s) => s.class?.code === selectedClass)
      : allSessions;

    const now = new Date();

    if (selectedPeriod === "week") {
      const weekStart = startOfWeekDate(now, { weekStartsOn: 0 }); // Sunday start
      ...
      // Placeholder: assuming semester = last 6 months
      const sixMonthsAgo = new Date(now.getFullYear(), now.getMonth() - 6, now.getDate());
      filtered = filtered.filter((s) => new Date(s.sessionDate) >= sixMonthsAgo);
    }

    return filtered;
  }, [allSessions, selectedClass, selectedPeriod]);
            ...
            <CardContent>
              {calendarEvents.length > 0 ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-500">Schedule some attendance sessions to see them here</p>
                </div>
              )}
            </CardContent>
          </Card>
