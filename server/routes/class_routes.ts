...
export function registerClassRoutes(app: Express) {
    ...
    try {
      ...
      for (const cls of classes) {
        if (cls.teacherId) {
          ...
          const teacherData = teacher.find(t => t.id === cls.teacherId);
          if (teacherData) {
            const user = await storage.getUser(teacherData.userId);
            if (user) {
        ...
        classesWithTeachers.push({ ...cls, teacher: null });
      }
  ...
  app.post("/api/classes", async (req: Request, res: Response) => {
    try {
      ...
      const classData = await storage.createClass({
        ...validatedData,
        teacherId: validatedData.teacherId ?? undefined, // converts null -> undefined
      });
  ...
  app.post("/api/classes/:classId/students/:studentId", async (req: Request, res: Response) => {
    try {
  ...
  app.delete("/api/classes/:classId/students/:studentId", authenticateToken, async (req: Request, res: Response) => {
    try {
